// Country list with flags for autocomplete UI.
// Flag emojis are intentional to satisfy UI requirement.
window.FCCountries = [
...
window.FCBuildCountryOptions = (datalistId) => {
  const list = document.getElementById(datalistId);
  if (!list || !Array.isArray(window.FCCountries)) return;
  list.innerHTML = window.FCCountries.map((c) => `<option value="${c.name} ${c.flag}"></option>`).join("");
};

window.FCNormalizeCountry = (value) =>
  String(value || "")
