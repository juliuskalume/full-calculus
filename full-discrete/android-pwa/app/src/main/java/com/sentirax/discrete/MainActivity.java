...
public class MainActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    googleWebClientId = resolveWebClientId();
    GoogleSignInOptions.Builder signInBuilder =
        new GoogleSignInOptions.Builder(GoogleSignInOptions.DEFAULT_SIGN_IN).requestEmail();
    if (!googleWebClientId.isEmpty()) {
      signInBuilder.requestIdToken(googleWebClientId);
    }
    @SuppressWarnings("deprecation")
    GoogleSignInOptions gso = signInBuilder.build();
  ...
  private void startGoogleSignIn() {
    if (googleWebClientId == null || googleWebClientId.isEmpty()) {
      sendAuthError("Google sign-in is not configured for this build.", null);
      return;
    }
